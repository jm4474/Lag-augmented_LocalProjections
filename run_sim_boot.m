...
%% List of specifications for the bootstrap

boot_ar_spec_nonaug = struct('estimator', 'ar',...   
                             'lag_aug', false,...
                             'biascorr', true); 
                             % Non-augmented specification used 
                             % for generating bootstrap samples
                             % (Estimation method is plug-in least squares)
                           

boot_ar_spec_lagaug = struct('estimator', 'ar',...
                             'lag_aug', true, ...
                             'biascorr', true); 
                             % Lag-augmented specification used 
                             % for generating bootstrap samples
                             % (Estimation method is plug-in least squares)                                   
...
specs{1} = struct('estimator', 'ar',...  %Estimation Method: "plug-in" ar
                  'lag_aug', false,....  %With or Without Lag-augmentation
                  'biascorr', false);    %With or Without Biascorrection
...
specs{2} = struct('estimator', 'ar',...
                  'lag_aug', false,...
                  'biascorr', true);
              
specs{3} = struct('estimator', 'ar',...
                  'lag_aug', true,...
                  'biascorr', false);
              
specs{4} = struct('estimator', 'ar',...
                  'lag_aug', true,...
                  'biascorr', true,...
                  'bootstrap', 'ar',...
                  'bootstrap_spec', boot_ar_spec_lagaug);
              
specs{5} = struct('estimator', 'ar',...
                  'lag_aug', true,...
                  'biascorr', true,...
                  'bootstrap', 'ar',...
                  'bootstrap_spec', boot_ar_spec_nonaug);
              
specs{6} = struct('estimator', 'lp', ...
                  'lag_aug', false,...
                  'har', true,...
                  'bootstrap', 'ar',...
                  'bootstrap_spec', boot_ar_spec_nonaug);
              
specs{7} = struct('estimator', 'lp',...
                  'lag_aug', true,...
                  'har', false,...
                  'bootstrap', 'ar',...
                  'bootstrap_spec', boot_ar_spec_nonaug);
              
specs{8} = struct('estimator', 'lp',...
                  'lag_aug', true,...
                  'har', false,....
                  'bootstrap', 'resid');
              
specs{9} = struct('estimator', 'lp',...
                  'lag_aug', true, ...
                  'har', false,...
                  'bootstrap', 'pair');
...
settings.boot_ar_restrict = [];      % Function that restricts coefficients
                                     % to stationary region when generating
                                     % AR bootstrap samples 
                                     % (if empty [], don't restrict)
...
settings.har_fct ....
         = @(Y,bw) ewc(Y,bw);        % HAR estimator 
                                     % (based on the EWC function 
                                     % on the 1Functions/ folder)
...
%     for i=1:numrep
        ...
        i_U = sim_garch(dgp.garch_alpha_0,...
                        dgp.garch_alpha_1,...
                        dgp.garch_beta_1,...
                        i_T);       % sim_garch is in the 1Functions folder
        ...
        for j=1:numspec
            ...
            [i_estims(j,:),...
             i_ses(j,:),   ...
             i_cis_dm,   ...
             i_cis_boot] = ir_estim(i_Y, specs{j}, settings);
                                        % ir_estim is in 1Functions
