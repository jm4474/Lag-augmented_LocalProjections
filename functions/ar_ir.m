function [irs, jacob] = ar_ir(beta,horzs)
    ...
    % Inputs:
    % beta      n x np  VAR coefficient matrices (A_1, ..., A_p)
    % horzs     1 x H   horizons of interest
    
    % Outputs:
    % irs       n x n x H           reduced-form impulse responses Theta_h at select horizons
    % jacob     n^2 x (n^2*p) x H   Jacobian of vec(Theta_h) at select horizons wrt. vec(beta)
    ...
    jacob = zeros(n^2,n^2*p,nh); % Will contain Jacobian of impulse responses at select horizons wrt. vec(beta)
    
    ir_p = [eye(n); zeros(n*(p-1),n)]; % Will contain last p impulse responses, stacked vertically
    jacob_p = zeros(n^2,n^2*p,p); % Will contain last p values of the Jacobian of vec(Theta_h) wrt. vec(beta)
    
    for h=1:maxh % Loop through horizons
        
        the_beta = beta(:,1:n*min(h,p));
        the_past_ir = ir_p(1:n*min(h,p),:);
        the_ir = the_beta*the_past_ir; % Impulse response at horizon h
        ...
        if nargout>1
            ...
            for l=1:min(h,p)
                the_jacob_p(:,1:n^2*min(h,p)) = the_jacob_p(:,1:n^2*min(h,p)) + kron_fast(beta(:,(l-1)*n+1:l*n),jacob_p(:,1:n^2*min(h,p),l),1);
            end
