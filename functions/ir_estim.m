function [irs, ses, cis_dm, cis_boot, betahat, res] = ir_estim(Y, p, horzs, varargin)
    ...
    if ~isempty(ip.Results.bootstrap)
        ...
        if strcmp(ip.Results.bootstrap, 'ar') % Recursive AR bootstrap specifications
            ...
            [~, ~, ~, betahat_ar, res_ar] = var_ir_estim(Y, ...
                                                  p,...
            ...
            pseudo_truth = var_ir(betahat_ar(1:p), horzs); % Pseudo-true impulse responses in bootstrap DGP
    
            for b=1:ip.Results.boot_num

                % Generate bootstrap sample based on (possibly lag-augmented) AR estimates
                Y_boot = var_boot(betahat_ar, res_ar, Y, p+ip.Results.boot_lag_aug, ip.Results.se_homosk, ip.Results.no_const);
            ...
            for h=1:nh % Treat each horizon separately
                ...
                for b=1:ip.Results.boot_num
                    ...
                    [the_linreg_betahat, the_linreg_ses] = linreg(Y_boot,X_boot,ip.Results.se_homosk,true); % Don't add extra intercept
